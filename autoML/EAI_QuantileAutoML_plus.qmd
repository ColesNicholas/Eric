...
# download from drive
drive_download(
  as_id('1Zd_na5wepsE3e_yKQEsGfDIScReDHkfj'),
  path = "eia.input.plus.xls",
  overwrite = TRUE)
```
...
x <- setdiff(names(train_df), 
                          c("date_time", "western_hub_dalmp_average",
                            "pjmiso_coalgen_hourly_average", "pjmiso_nucleargen_hourly_average",
                            "aep_dayton_hub_dalmp_average", "n_illinois_hub_dalmp_average",
                            "dominion_hub_dalmp_average", "eastern_hub_dalmp_average"))
...
dl_55_lo <- h2o.deeplearning(
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.225
)

dl_55_hi <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.775)

# 45
dl_45_lo <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.275
)

dl_45_hi <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.725)

# 35
dl_35_lo <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.325
)

dl_35_hi <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.675)

# 25
dl_25_lo <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.375
)

dl_25_hi <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.625)

# 15
dl_15_lo <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
  ...
  categorical_encoding = "OneHotInternal",
  distribution     = "quantile",
  quantile_alpha   = 0.425
)

dl_15_hi <- h2o.deeplearning(
  x = x, y = y,
  training_frame   = trainvalid_h2o,
  nfolds = 5,
  fold_assignment = "Modulo",
...
predicted.value <- 
  c(ev.t.input$lo99,
    ev.t.input$lo95, ev.t.input$lo85,
    ev.t.input$lo75, ev.t.input$lo65, 
    ev.t.input$lo55, ev.t.input$lo45,
    ev.t.input$lo35, ev.t.input$lo25,
    ev.t.input$lo15,
    ev.t.input$med,
    ev.t.input$hi15,
    ev.t.input$hi25, ev.t.input$hi35, 
    ev.t.input$hi45, ev.t.input$hi55, 
    ev.t.input$hi65, ev.t.input$hi75,
    ev.t.input$hi85, ev.t.input$hi95,
    ev.t.input$hi99)

ggplot(data = cbind(p = predicted.value,
                    q = quantile),
       aes(x = quantile, y = predicted.value)) +
  geom_point() +
  geom_line() +
  geom_smooth(se = F)

# Trapezoidal integration over p
expected_value.buy <-
  lapply(X = seq(20, 60, by = .05),
         function(bid){
           g <-
             predicted.value - bid 
           
           expected_value <-
             sum((head(g, -1) + tail(g, -1)) / 2 * diff(quantile))
  ...
  bind_rows() %>% 
  mutate(rational = 
           if_else(expected_value > 0,
                   'yes',
                   'no'))

note <- 
  paste0('max acceptable buy: $',
    expected_value.buy[expected_value.buy$rational == 'yes', ]$bid %>%
      max())

evb <-
  ggplot(data = expected_value.buy,
       aes(x = bid, y = expected_value,
           colour = rational)) +
  geom_point() +
  labs(title = note,
       x = 'buy price',
...
note <- 
  paste0('min acceptable sell: $',
    expected_value.sell[expected_value.sell$rational == 'yes', ]$bid %>%
      min())

evs <- 
  ggplot(data = expected_value.sell,
       aes(x = bid, y = expected_value,
           colour = rational)) +
  geom_point() +
  labs(title = note,
...
```{r}
# for debugging
# d <- as.character(results[!is.na(results$western_hub_dalmp_average), ]$date_time)

ev.res <- 
  lapply(
  as.character(results[!is.na(results$western_hub_dalmp_average), ]$date_time),
               function(d){
                 ...
                 # filter and extract information
                  ...
                  actual.value.sell <-
                    lapply(X = seq(20, 60, by = .05),
                           function(bid){
                             actual.val <-
                               bid - ev.t.input$western_hub_dalmp_average
                             
                             list(western_hub_dalmp_average = ev.t.input$western_hub_dalmp_average,
                                  eai.pred = ev.t.input$yhat,
                                  actual.val = actual.val)
                             }
                           ) %>% 
                    bind_rows()
                  
                  cbind(expected_value.sell, 
                        actual.value.sell,
                        d) %>% 
                    return()
               }
...
# the typical expected value estimate were inflated by $2.58
err <- ev.res.full$expected_value - ev.res.full$actual.val
per <- median(err) %>% round(2)
...
ggplot(ev.res.full,
  ...
  labs(title = 
         paste0(
           ev.res.full$d %>% head(1),
           " to ",
           ev.res.full$d %>% tail(1),
           "; median error = ",
           per
           )
       )

# when limiting to a more plausible scenario: still 15 cents off
ev.res.full %>%
  filter(abs(eai.pred - bid) < 1) %>%
  mutate(err = expected_value - actual.val) %>% 
#  filter(rational == 'yes') %>% 
  summarise(med.err = median(err),
            
            med.ev = median(expected_value),
            max.ev = max(expected_value),
            min.ev = min(expected_value),
            
            med.av = median(actual.val),
            max.av = max(actual.val),
            min.av = min(actual.val))
```
