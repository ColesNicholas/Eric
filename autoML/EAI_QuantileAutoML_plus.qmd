...
ggplot(density_df, aes(x = predicted_value, y = density)) +
  ...
  ylab('density (larger == more probable)')

# new stuff

purchase_price <- 32
payoff <- function(actual) actual - purchase_price # gain/loss

# Compute payoff at each quantile
g <- payoff(predicted.value)
...
note <- 
  paste0('max acceptable buy: $',
    expected_value.anal[expected_value.anal$rational == 'yes', ]$bid %>%
      max())

ggplot(data = expected_value.anal,
