...
y <- "aep_dayton_hub_dalmp_average"
```
...
results <- results %>%
  mutate(abs.err = abs(aep_dayton_hub_dalmp_average - pred))
...
ts <- 
  ...
  # plot actual
  geom_point(aes(y = aep_dayton_hub_dalmp_average), 
             size = 1.2, 
             alpha = 0.7) +
  geom_line(aes(y = aep_dayton_hub_dalmp_average), 
            linewidth = 1) +
  ylab('DA-LMP') +
  xlab('date')+
...
summary <-
  ...
  pivot_wider(names_from = 'bound',
              values_from = 'value') %>% 
  mutate(abs.err = abs(aep_dayton_hub_dalmp_average - yhat),
         hit = 
           if_else(aep_dayton_hub_dalmp_average > lo & aep_dayton_hub_dalmp_average < hi,
                   true = 1,
                   false = 0))
