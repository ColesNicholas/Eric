...
df <- 
  ...
  # for new experiment, focus on delta
  rowwise %>% 
  mutate(delta = 
            western_hub_dalmp_average - aep_dayton_hub_dalmp_average) %>% 
  ungroup()
```
  ...
  add_lags(data = df, 
           ...
           exog = setdiff(colnames(df), 
                          c("date_time", "delta",
                            ...
                            "aep_dayton_hub_dalmp_average", "western_hub_dalmp_average",
...
x <- setdiff(names(train_df), 
             c("date_time", "western_hub_dalmp_average",
               ...
               "dominion_hub_dalmp_average", "eastern_hub_dalmp_average",
               "western_hub_dalmp_average", "delta"))
...
summary <-
  ...
  pivot_wider(names_from = 'bound',
              values_from = 'value') %>% 
  mutate(abs.err = abs(delta - yhat),
         hit = 
           if_else(delta > lo & delta < hi,
                   true = 1,
                   false = 0))
